...
htmx.onLoad((content) => {
  ...
  htmx.findAll(content, ".summary").forEach((element) => {
    htmx.on(`#${element.id}`, "click", () =>
      htmx.toggleClass(htmx.find(`#${element.id}-reveal`), "hide"),
    );
  });

  htmx.findAll("#direct-debit-button").forEach(() => {
    return showOrHideDirectDebitButton();
  });
  ...
  htmx.findAll(".show-manager-override-field").forEach((element) => {
    element.addEventListener("click", () =>
      htmx.removeClass(htmx.find("#manager-override-field"), "hide"),
    );
  });

  htmx.findAll(".hide-manager-override-field").forEach((element) => {
    element.addEventListener("click", () =>
      htmx.addClass(htmx.find("#manager-override-field"), "hide"),
    );
  });
  ...
  htmx.findAll(".moj-banner--success").forEach((element) => {
    element.addEventListener("click", () =>
      htmx.addClass(htmx.find(".moj-banner--success"), "hide"),
    );
  });
  ...
  htmx.findAll("#invoice-type").forEach((element) => {
    element.addEventListener(
      ...
      function () {
        ...
        document
          .querySelector("#amount-field-input #amount")
          .setAttribute("disabled", "true");
        document
          .querySelector("#start-date-field-input #startDate")
          .setAttribute("disabled", "true");
        document
          .querySelector("#end-date-field-input #endDate")
          .setAttribute("disabled", "true");
        document
          .querySelector("#raised-date-field-input #raisedDate")
          .setAttribute("disabled", "true");
        document
          .querySelector("#raised-year-field-input #raisedYear")
          .setAttribute("disabled", "true");
        document
          .querySelector("#supervision-level-field-input #supervisionLevel")
          .setAttribute("disabled", "true");
        ...
        switch (invoiceTypeSelect.value) {
          ...
          case "SO":
            htmx.removeClass(
              htmx.find("#supervision-level-field-input"),
              "hide",
            );
            document
              .querySelector("#supervision-level-field-input #supervisionLevel")
