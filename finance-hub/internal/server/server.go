...
type ApiClient interface {
	AddDirectDebit(context.Context, int, api.AccountDetails) error
...
func New(logger *slog.Logger, client *api.Client, templates map[string]*template.Template, envs Envs) http.Handler {
	...
	handleMux("POST /clients/{clientId}/payment-method/add", &SubmitPaymentMethodHandler{&route{client: client, tmpl: templates["add-direct-debit.gotmpl"], partial: "error-summary"}})
