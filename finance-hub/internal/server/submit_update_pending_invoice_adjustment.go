...
func (h *SubmitUpdatePendingInvoiceAdjustmentHandler) render(v AppVars, w http.ResponseWriter, r *http.Request) error {
	...
	if err == nil {
		w.Header().Add("HX-Redirect", fmt.Sprintf("%s/clients/%d/invoice-adjustments?success=%s-invoice-adjustment[%s]", v.EnvironmentVars.Prefix, ctx.ClientId, strings.ToLower(status), strings.ToUpper(r.PathValue("adjustmentType"))))
	} else {
		var (
