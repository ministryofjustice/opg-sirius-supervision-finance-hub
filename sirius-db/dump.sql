...
(
    ...
    batchnumber        integer,
    cachedoutstandingbalance   varchar(255)      not null,
    cachedcreditbalance varchar(255)      not null
);

comment
on column finance_client.payment_method is '(DC2Type:refdata)';
...
comment
on column fee_reduction.type is '(DC2Type:refdata)';

comment
on column fee_reduction.evidencetype is '(DC2Type:refdata)';
...
comment
on column invoice.amount is '(DC2Type:money)';

comment
on column invoice.supervisionlevel is '(DC2Type:refdata)';

comment
on column invoice.cacheddebtamount is '(DC2Type:money)';
...
comment
on column invoice_email_status.status is '(DC2Type:refdata)';

comment
on column invoice_email_status.templateid is '(DC2Type:refdata)';
...
comment
on column invoice_fee_range.supervisionlevel is '(DC2Type:refdata)';

comment
on column invoice_fee_range.amount is '(DC2Type:money)';
...
comment
on column ledger.amount is '(DC2Type:money)';

comment
on column ledger.type is '(DC2Type:refdata)';

comment
on column ledger.status is '(DC2Type:refdata)';

comment
on column ledger.bankaccount is '(DC2Type:refdata)';
...
comment
on column ledger_allocation.amount is '(DC2Type:money)';

comment
on column ledger_allocation.status is '(DC2Type:refdata)';
...
comment
on column rate.amount is '(DC2Type:money)';

alter table rate
    owner to api;

INSERT INTO finance_client (id, client_id, sop_number, payment_method, batchnumber, cachedoutstandingbalance, cachedcreditbalance) VALUES (1, 1, '99999999', 'Demanded', '555', '22.22', '1.01');
INSERT INTO finance_client (id, client_id, sop_number, payment_method, batchnumber, cachedoutstandingbalance, cachedcreditbalance) VALUES (2, 2, '88888888', 'Direct Debit', '0', '0', '0');