...
WITH ledger_sums AS (
    SELECT
        ...
        SUM(CASE WHEN l.status = 'CONFIRMED' AND la.status NOT IN ('PENDING', 'UNALLOCATED') THEN la.amount ELSE 0 END) AS received
    FROM ledger_allocation la
             JOIN ledger l ON la.ledger_id = l.id
...
SELECT
    i.amount AS initial,
    i.amount - COALESCE(ls.received, 0) AS outstanding,
    i.feetype,
