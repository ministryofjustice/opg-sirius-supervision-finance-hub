...
func (suite *IntegrationSuite) Test_invoice_adjustments() {
	...
	suite.seeder.CreateOrder(ctx, client1ID, "ACTIVE")
	invoiceId, client3InvoiceRef := suite.seeder.CreateInvoice(ctx, client3ID, shared.InvoiceTypeGS, valToPtr("200.00"), today.StringPtr(), today.StringPtr(), today.StringPtr(), nil, nil)
	...
	assert.Equal(suite.T(), "24/25", results[1]["Financial Year"], "Financial Year - client 3")
	...
	assert.Equal(suite.T(), "10486000", results[2]["Revenue cost centre"], "Cost centre - client 2")
	assert.Equal(suite.T(), "Allocations, HW & SIS BISD", results[2]["Revenue cost centre description"], "Cost centre description - client 2")
	assert.Equal(suite.T(), "4481102107", results[2]["Revenue account code"], "Account code - client 2")
	assert.Equal(suite.T(), "INC - RECEIPT OF FEES AND CHARGES - GUARDIANSHIP FEE REMISSION", results[2]["Revenue account descriptions"], "Account code description - client 2")
	...
	assert.Equal(suite.T(), "24/25", results[2]["Financial Year"], "Financial Year - client 2")
	assert.Equal(suite.T(), time.Now().Format("2006-01-02"), results[2]["Approved date"], "Approved date - client 2")
	assert.Equal(suite.T(), "100.00", results[2]["Adjustment amount"], "Adjustment amount - client 2")
