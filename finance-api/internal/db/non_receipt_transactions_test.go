...
func (suite *IntegrationSuite) Test_non_receipt_transactions() {
	...
	// one client with one invoice and an exemption - credit
	...
	assert.Equal(suite.T(), "10000", results[0]["Credit"], "Credit - client 1 invoice 1")
	assert.Equal(suite.T(), fmt.Sprintf("AD Rem/Exem [%s]", yesterday.Date().Format("02/01/2006")), results[0]["Line description"], "Line description - client 1 invoice 1")
}
