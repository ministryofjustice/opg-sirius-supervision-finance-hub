...
func (suite *IntegrationSuite) Test_fee_chase() {
	...
	client3ID := suite.seeder.CreateClient(ctx, "Patrick", "Stewart", "87651234", "4312")
	...
	client4ID := suite.seeder.CreateClient(ctx, "Ian", "McGregor", "12348765", "4132")
	...
	//assert.Equal(suite.T(), "=UNICHAR(163)&190.99", results[0]["Total_debt"], "Total debt - client 1")
	assert.Equal(suite.T(), "B2000001/0", results[0]["Invoice1"], "Invoices - client 1")
	//assert.Equal(suite.T(), "=UNICHAR(163)&90.99", results[0]["Amount1"], "Invoices - client 1")
	assert.Equal(suite.T(), "AD000001/25", results[0]["Invoice2"], "Invoices - client 1")
	//assert.Equal(suite.T(), "=UNICHAR(163)&100.00", results[0]["Amount2"], "Invoices - client 1")
	...
	//assert.Equal(suite.T(), "=UNICHAR(163)&70.21", results[1]["Total_debt"], "Total debt - client 2")
	assert.Equal(suite.T(), "S2000002/0", results[1]["Invoice1"], "Invoices - client 2")
	//assert.Equal(suite.T(), "=UNICHAR(163)&70.21", results[1]["Amount1"], "Invoices - client 2")

	// client 4
	assert.Equal(suite.T(), "12348765", results[2]["Case_no"], "Case name - client 4")
	assert.Equal(suite.T(), "7", results[2]["Client_no"], "Client number - client 4")
	assert.Equal(suite.T(), "", results[2]["Client_title"], "Client title - client 4")
	assert.Equal(suite.T(), "Ian", results[2]["Client_forename"], "Client forename - client 4")
	assert.Equal(suite.T(), "McGregor", results[2]["Client_surname"], "Client surname - client 4")
	assert.Equal(suite.T(), "Yes", results[2]["Do_not_chase"], "Do not chase - client 4")
	assert.Equal(suite.T(), "Demanded", results[2]["Payment_method"], "Payment method - client 4")
	assert.Equal(suite.T(), "PRO", results[2]["Deputy_type"], "Deputy type - client 4")
	assert.Equal(suite.T(), "", results[2]["Deputy_no"], "Deputy number - client 4")
	assert.Equal(suite.T(), "Yes", results[2]["Airmail"], "Airmail required - client 4")
	assert.Equal(suite.T(), "", results[2]["Deputy_title"], "Deputy title - client 4")
	assert.Equal(suite.T(), "No", results[2]["Deputy_Welsh"], "Deputy Welsh - client 4")
	assert.Equal(suite.T(), "No", results[2]["Deputy_Large_Print"], "Deputy large print - client 4")
	assert.Equal(suite.T(), "Jason Statham", results[2]["Deputy_name"], "Deputy name - client 4")
	assert.Equal(suite.T(), "", results[2]["Email"], "Deputy email - client 4")
	assert.Equal(suite.T(), "94 Fake Avenue", results[2]["Address1"], "Deputy address 1 - client 4")
	assert.Equal(suite.T(), "", results[2]["Address2"], "Deputy address 2 - client 4")
	assert.Equal(suite.T(), "", results[2]["Address3"], "Deputy address 3 - client 4")
	assert.Equal(suite.T(), "Realton", results[2]["City_Town"], "Deputy town - client 4")
	...
	//assert.Equal(suite.T(), "=UNICHAR(163)&100.00", results[2]["Total_debt"], "Total debt - client 4")
	assert.Equal(suite.T(), "AD000004/25", results[2]["Invoice1"], "Invoices - client 4")
	//assert.Equal(suite.T(), "=UNICHAR(163)&100.00", results[2]["Amount1"], "Invoices - client 4")
}
