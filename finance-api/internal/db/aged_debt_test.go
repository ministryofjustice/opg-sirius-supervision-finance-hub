...
func (suite *IntegrationSuite) Test_aged_debt() {
	...
	_, c1i1Ref := suite.seeder.CreateInvoice(ctx, client1ID, shared.InvoiceTypeGA, nil, twoMonthsAgo.StringPtr(), nil, nil, nil)
	...
	suite.seeder.CreateFeeReduction(ctx, client2ID, shared.FeeReductionTypeRemission, strconv.Itoa(fourYearsAgo.Sub(1, 0, 0).Date().Year()), 2, "A reduction")
	...
	suite.seeder.CreateFeeReduction(ctx, client2ID, shared.FeeReductionTypeRemission, strconv.Itoa(oneYearAgo.Sub(1, 0, 0).Date().Year()), 2, "A reduction")
