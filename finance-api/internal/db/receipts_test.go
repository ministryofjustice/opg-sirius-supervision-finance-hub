...
func (suite *IntegrationSuite) Test_receipts() {
	...
	suite.seeder.ReversePayment(ctx, courtRef2, courtRef3, "150.00", yesterday.Date(), yesterday.Date(), shared.TransactionTypeOnlineCardPayment)
	...
	assert.Equal(suite.T(), "1816102003", results[8]["Receivables account code"], "Line 9: Receivables account code")
	assert.Equal(suite.T(), "CA - TRADE RECEIVABLES - SIRIUS SUPERVISION CONTROL ACCOUNT", results[8]["Account code description"], "Line 9: Account code description")
	...
	assert.Equal(suite.T(), "1816102005", results[9]["Receivables account code"], "Line 10: Receivables account code")
	assert.Equal(suite.T(), "CA - TRADE RECEIVABLES - ON ACCOUNT RECEIPTS – SIRIUS SUPERVISION", results[9]["Account code description"], "Line 10: Account code description")
