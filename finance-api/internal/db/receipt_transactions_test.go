...
func (suite *IntegrationSuite) Test_receipt_transactions() {
	...
	suite.seeder.CreatePayment(ctx, 150, yesterday.Date(), "12345678", shared.TransactionTypeMotoCardPayment, yesterday.Date())
	suite.seeder.CreatePayment(ctx, 250, yesterday.Date(), "12345678", shared.TransactionTypeSupervisionBACSPayment, yesterday.Date())
	...
	assert.Equal(suite.T(), "00000", results[0]["Spare"], "Spare - BACS Payments")
	assert.Equal(suite.T(), "", results[0]["Debit"], "Debit - BACS Payments")
	assert.Equal(suite.T(), "250", results[0]["Credit"], "Credit - BACS Payments")
	...
	assert.Equal(suite.T(), "000000", results[1]["Spare"], "Spare - BACS Payments 2")
	assert.Equal(suite.T(), "250", results[1]["Debit"], "Debit - BACS Payments 2")
	assert.Equal(suite.T(), "", results[1]["Credit"], "Credit - BACS Payments 2")
	assert.Equal(suite.T(), fmt.Sprintf("BACS Payment [%s]", today.Date().Format("02/01/2006")), results[1]["Line description"], "Line description - BACS Payments 2")
	...
	assert.Equal(suite.T(), "00000", results[2]["Spare"], "Spare - Moto Payments")
	assert.Equal(suite.T(), "", results[2]["Debit"], "Debit - Moto Payments")
	assert.Equal(suite.T(), "150", results[2]["Credit"], "Credit - Moto Payments")
	assert.Equal(suite.T(), fmt.Sprintf("MOTO (Phone) Card Payment [%s]", today.Date().Format("02/01/2006")), results[2]["Line description"], "Line description - Moto Payments")
	...
	assert.Equal(suite.T(), "000000", results[3]["Spare"], "Spare - MOTO Payments 2")
	assert.Equal(suite.T(), "150", results[3]["Debit"], "Debit - MOTO Payments 2")
	assert.Equal(suite.T(), "", results[3]["Credit"], "Credit - MOTO Payments 2")
	assert.Equal(suite.T(), fmt.Sprintf("MOTO (Phone) Card Payment [%s]", today.Date().Format("02/01/2006")), results[3]["Line description"], "Line description - MOTO Payments 2")
}
