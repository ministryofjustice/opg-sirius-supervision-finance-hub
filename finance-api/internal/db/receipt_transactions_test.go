...
func (suite *IntegrationSuite) Test_receipt_transactions_reversals() {
	...
	assert.Equal(suite.T(), fmt.Sprintf("Refund [%s]", twoMonthsAgo.Date().Format("02/01/2006")), results[12]["Line description"], "Line description - Refund Reversal Debit")
	...
	assert.Equal(suite.T(), fmt.Sprintf("Refund [%s]", twoMonthsAgo.Date().Format("02/01/2006")), results[13]["Line description"], "Line description - Refund Reversal Credit")
}
