...
func (suite *IntegrationSuite) Test_all_refunds() {
	...
	suite.seeder.ProcessApprovedRefunds(ctx, []int32{refund1ID, refund3ID}, twoDaysAgo.Date())
	...
	assert.Equal(suite.T(), "22222222", results[0]["Court reference"], "Court reference - client 1")
	assert.Equal(suite.T(), "150.00", results[0]["Amount"], "Amount - client 1")
	assert.Equal(suite.T(), yesterday.String(), results[0]["Create date"], "Create date - client 1")
	assert.Equal(suite.T(), "Johnny Test", results[0]["Created by"], "Created by - client 1")
	assert.Equal(suite.T(), "", results[0]["Approved by"], "Approved by - client 1")
	assert.Equal(suite.T(), "PENDING", results[0]["Status"], "Status - client 1")
	assert.Equal(suite.T(), yesterday.String(), results[0]["Status Date"], "Status Date - client 1")

	assert.Equal(suite.T(), "33333333", results[1]["Court reference"], "Court reference - client 2")
	assert.Equal(suite.T(), "150.00", results[1]["Amount"], "Amount - client 2")
	assert.Equal(suite.T(), yesterday.String(), results[1]["Create date"], "Create date - client 2")
	assert.Equal(suite.T(), "Johnny Test", results[1]["Created by"], "Created by - client 2")
	assert.Equal(suite.T(), "Johnny Test", results[1]["Approved by"], "Approved by - client 2")
	assert.Equal(suite.T(), "APPROVED", results[1]["Status"], "Status - client 2")
	assert.Equal(suite.T(), today.String(), results[1]["Status Date"], "Status Date - client 2")

	assert.Equal(suite.T(), "66666666", results[2]["Court reference"], "Court reference - client 3")
	assert.Equal(suite.T(), "150.00", results[2]["Amount"], "Amount - client 3")
	assert.Equal(suite.T(), threeDaysAgo.String(), results[2]["Create date"], "Create date - client 3")
	assert.Equal(suite.T(), "Johnny Test", results[2]["Created by"], "Created by - client 3")
	assert.Equal(suite.T(), "Johnny Test", results[2]["Approved by"], "Approved by - client 3")
	assert.Equal(suite.T(), "FULFILLED", results[2]["Status"], "Status - client 3")
	assert.Equal(suite.T(), yesterday.String(), results[2]["Status Date"], "Status Date - client 3")

	assert.Equal(suite.T(), "11111111", results[3]["Court reference"], "Court reference - client 4")
	assert.Equal(suite.T(), "150.00", results[3]["Amount"], "Amount - client 4")
	assert.Equal(suite.T(), fourDaysAgo.String(), results[3]["Create date"], "Create date - client 4")
	assert.Equal(suite.T(), "Johnny Test", results[3]["Created by"], "Created by - client 4")
	assert.Equal(suite.T(), "Johnny Test", results[3]["Approved by"], "Approved by - client 4")
	assert.Equal(suite.T(), "PROCESSING", results[3]["Status"], "Status - client 4")
	assert.Equal(suite.T(), twoDaysAgo.String(), results[3]["Status Date"], "Status Date - client 4")
}
