...
func (suite *IntegrationSuite) Test_processReversals() {
	...
	seeder.SeedData(
		...
		"INSERT INTO ledger VALUES (18, 'test 16', '2025-01-02 15:32:10', '', 5000, 'payment 16 being reversed', 'SUPERVISION CHEQUE PAYMENT', 'CONFIRMED', 16, NULL, NULL, NULL, '2025-01-02', NULL, NULL, NULL, NULL, '2025-01-02', 1, 101);",
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			rows, _ := seeder.Query(suite.ctx,
				`SELECT COALESCE(l.amount, 0), l.type, l.status, l.datetime, l.bankdate, COALESCE(la.amount, 0), la.status, l.finance_client_id, la.invoice_id, COALESCE(l.pis_number, 0)
						FROM ledger l
