...
func (suite *IntegrationSuite) Test_getPaymentDetails() {
	tests := []struct {
		...
		{
			"Amount parse error",
			[]string{"87654321-abcdefg-garbleddata", "01-02-2025 11:33:55", "oops!"},
			shared.ReportTypeUploadPaymentsOnlineCard.Key(),
			shared.NewDate("2025-02-10"),
			shared.TransactionTypeOnlineCardPayment.Key(),
			1,
			shared.PaymentDetails{},
			map[int]string{1: "AMOUNT_PARSE_ERROR"},
		},
		{
			"MOTO card record date parse error",
			[]string{"87654321-abcdefg-garbleddata", "oh no!", "2"},
			shared.ReportTypeUploadPaymentsOnlineCard.Key(),
			shared.NewDate("2025-02-10"),
			shared.TransactionTypeOnlineCardPayment.Key(),
			2,
			shared.PaymentDetails{},
			map[int]string{2: "DATE_TIME_PARSE_ERROR"},
		},
	}
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
