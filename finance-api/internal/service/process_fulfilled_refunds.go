...
func (s *Service) validateFulfilledRefundsLine(ctx context.Context, details shared.FulfilledRefundDetails, index int, failedLines *map[int]string) bool {
	id, _ := s.store.GetProcessingRefund(ctx, store.GetProcessingRefundParams{
		CourtRef:      details.CourtRef,
		Amount:        details.Amount,
		AccountName:   details.AccountName,
		AccountNumber: details.AccountNumber,
