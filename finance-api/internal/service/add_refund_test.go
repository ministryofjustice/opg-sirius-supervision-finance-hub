...
func (suite *IntegrationSuite) TestService_AddRefund() {
	...
	rows := seeder.QueryRow(ctx, "SELECT r.raised_date, r.amount, r.status, r.notes, r.created_by, b.name, b.account, b.sort_code FROM refund r JOIN bank_details b ON r.id = b.refund_id WHERE r.client_id = $1", clientID)
